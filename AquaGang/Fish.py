...
available_pond_assets = {"local-pond", "foreign-pond", "mega-pond", "doo-pond", "khor-pond", "matrix-pond"}
...
class Fish(pygame.sprite.Sprite):
    def __init__(
        self, pos_x=None, pos_y=None, genesis="AquaGang-pond", parent=None, data: FishData = None
    ):
        super().__init__()
    ...
    def loadSprite(self):
        path = "local-pond"
        if (
            self.fishData.genesis != "AquaGang-pond"
...
class FishGroup(pygame.sprite.Group):
    ...
    def getFishes(self):
        fishes = []
        for fish in self.fishes["AquaGang-pond"].values():
            fishes.append(fish)
        return fishes
